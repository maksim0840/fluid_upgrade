...
struct FastFixed {
    ...
    constexpr FastFixed(T value, double accuracy_value = 0.0) {
        // Проверка K
        if (K < 0 || N <= K) {
        ...
        else if constexpr(IS_SAME_V_INT || IS_SAME_V_FAST_INT) {
    ...
    constexpr FastFixed() : FastFixed(0.0) {} // double = 0 по умолчанию

     // Создаёт объект Fixed из сырого значения, преобразуя его в тип текущего variant
    template<typename T>
    constexpr FastFixed from_raw(T other_value) const {
